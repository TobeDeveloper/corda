...
class CordaRPCClientTest : NodeBasedTest() {
    ...
    fun `close-send deadlock and premature shutdown on empty observable`() {
        ...
        val flowHandle = connection!!.proxy.startTrackedFlow(::CashIssueFlow,
                20.DOLLARS, node.info.legalIdentity, OpaqueBytes.of(0), node.info.legalIdentity
        )
    ...
    fun `get cash balances`() {
        ...
        val flowHandle = proxy.startFlow(::CashIssueFlow,
                123.DOLLARS, node.info.legalIdentity, OpaqueBytes.of(0), node.info.legalIdentity
        )
    ...
    fun `flow initiator via RPC`() {
        ...
        node.services.startFlow(CashIssueFlow(2000.DOLLARS, nodeIdentity, OpaqueBytes.of(0), nodeIdentity), FlowInitiator.Shell).resultFuture.getOrThrow()
        proxy.startFlow(::CashIssueFlow,
                123.DOLLARS,
                nodeIdentity, OpaqueBytes.of(0),
                nodeIdentity
