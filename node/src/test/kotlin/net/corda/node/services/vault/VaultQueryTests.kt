...
class VaultQueryTests : TestDependencyInjectionBase() {
    ...
    fun `unconsumed linear heads for linearId without external Id`() {
        database.transaction {
            ...
            val criteria = LinearStateQueryCriteria(linearIdUUID = listOf(linearIds.first(), linearIds.last()))
            val results = vaultQuerySvc.queryBy<LinearState>(criteria)
    ...
    fun `unconsumed linear heads for linearId with external Id`() {
        database.transaction {
            ...
            val criteria = LinearStateQueryCriteria(linearIdUUID = linearIds)
            val results = vaultQuerySvc.queryBy<LinearState>(criteria)
            assertThat(results.states).hasSize(2)
        }
    }
    ...
    fun `all linear states for a given id`() {
        database.transaction {
            ...
            val linearStateCriteria = LinearStateQueryCriteria(linearIdUUID = listOf(linearId.id), status = Vault.StateStatus.ALL)
    ...
    fun `all linear states for a given id sorted by uuid`() {
        database.transaction {
            ...
            // should now have 1 UNCONSUMED & 3 CONSUMED state refs for Linear State with "TEST"
            val linearStateCriteria = LinearStateQueryCriteria(linearIdUUID = linearStates.map { it.state.data.linearId.id }, status = Vault.StateStatus.ALL)
            val vaultCriteria = VaultQueryCriteria(status = Vault.StateStatus.ALL)
    ...
    fun `unconsumed deal states sorted`() {
        database.transaction {
            ...
            val linearStateCriteria = LinearStateQueryCriteria(linearIdUUID = listOf(uid))
            val dealStateCriteria = LinearStateQueryCriteria(linearIdRef = listOf("123", "456", "789"))
    ...
    fun `return consumed linear states for a given id`() {
        database.transaction {
            ...
            // should now have 1 UNCONSUMED & 3 CONSUMED state refs for Linear State with "TEST"
            val linearStateCriteria = LinearStateQueryCriteria(linearIdUUID = txns.states.map { it.state.data.linearId.id }, status = Vault.StateStatus.CONSUMED)
    ...
    fun `unconsumed deals for ref`() {
        database.transaction {
            ...
            val criteria = LinearStateQueryCriteria(linearIdRef = listOf("456", "789"))
    ...
    fun `latest unconsumed deals for ref`() {
        database.transaction {
            ...
            val criteria = LinearStateQueryCriteria(linearIdRef = listOf("456"))
            val results = vaultQuerySvc.queryBy<DealState>(criteria)
            assertThat(results.states).hasSize(1)
        }
    }
