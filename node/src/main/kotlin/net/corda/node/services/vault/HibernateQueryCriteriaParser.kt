...
class HibernateQueryCriteriaParser(val contractType: Class<out ContractState>,
    ...
    override fun parseCriteria(criteria: QueryCriteria.LinearStateQueryCriteria) : Collection<Predicate> {
        ...
        // linear ids UUID
        criteria.linearIdUUID?.let {
            val uniqueIdentifiers = criteria.linearIdUUID as List<UUID>
            predicateSet.add(criteriaBuilder.and(vaultLinearStates.get<UUID>("uuid").`in`(uniqueIdentifiers.map { it })))
        }

        // linear ids externalRefs
        criteria.linearIdRef?.let {
            val externalIds = criteria.linearIdRef as List<String>
